...
curl -i -X POST -H "Accept:application/json" -H "Content-Type:application/json" $CONNECT_HOST:$CONNECT_PORT/connectors/ -d "{
  ...
  \"config\": {
    ...
    \"collection.include.list\": \"$MONGO_PREFIX-auth.*,$MONGO_PREFIX-career.*,$MONGO_PREFIX-conjoint.*,$MONGO_PREFIX-content.*,$MONGO_PREFIX-context.*,$MONGO_PREFIX-domain.*,$MONGO_PREFIX-education.*,$MONGO_PREFIX-essential[.]sagas,$MONGO_PREFIX-financial.*,$MONGO_PREFIX-general.*,$MONGO_PREFIX-identity.*,$MONGO_PREFIX-logistic.*,$MONGO_PREFIX-special[.]files,$MONGO_PREFIX-touch.*,$MONGO_PREFIX-thing.*\"
  }
}"