...
description: "Official definition of the Model Context Protocol (MCP) — the standard governing how AI agents discover, read, understand, query, and interact with structured information inside the Wenex platform and its ecosystem. Enhanced guidance for AI agents on secure handling and user assistance with Auth Personal Tokens (APT)."
...
AI agents are explicitly designed to **assist human users** by leveraging the user’s own **Auth Personal Token (APT)**. This allows agents to act securely on the user’s behalf within the Coworkers Space while respecting all platform access-control rules.
...
| Term                          | Definition |
|-------------------------------|------------|
| **Auth Personal Token (APT)** | A long-lived personal access token issued to a human user. It authenticates AI agents and clients for core platform operations. **AI agents must always use the user’s own APT** to act on their behalf. Agents are responsible for guiding users through obtaining, providing, and (if needed) revoking their APT. See section 2.3 for detailed assistance protocols. |
...
### 2.3 AI Agent Assistance with User APT Tokens

AI agents are required to **help users securely obtain and use their Auth Personal Token (APT)** so the agent can interact with the Wenex platform on the user’s behalf. Follow these mandatory guidelines:

1. **User Guidance for Obtaining an APT**  
   Direct the user to the official flow described in the MCP resource `docs://core/auth-specification`. The agent must explain in plain language:
   - What an APT is and why it is needed.
   - That the token grants the agent permission to act as the user (with the same permissions the user has).
   - That the token should be treated with the same care as a password.

2. **Secure Token Input**  
   Request the APT only through secure, client-provided input mechanisms (never via public chat if a secure channel is available). Once received, the agent stores the token **only for the current session** unless the user explicitly authorizes longer-term secure storage.

3. **Token Usage in Operations**  
   Every MCP request made by the agent **must include the user’s APT** for authentication (typically as an `Authorization: Bearer <APT>` header or equivalent client SDK method). The agent inherits the user’s `uid`, zones, and grants automatically.

4. **Error Handling & Recovery**  
   - If a request fails due to an invalid, expired, or revoked APT, the agent must clearly inform the user and immediately guide them through generating a new token.
   - Never retry operations with an expired token.

5. **Revocation Assistance**  
   If the user wishes to revoke the current APT (e.g., when ending a session or for security reasons), the agent must guide them to the revocation steps in `docs://core/auth-specification`.

6. **Security Best Practices**  
   - Never log, display, or transmit the APT except in the authenticated API call.
   - Never share the APT with other agents or clients without explicit user consent.
   - Remind the user that they can revoke the token at any time from their client dashboard.
...
The platform access control model is **based on** ABAC (Attribute-Based Access Control). A grant is the smallest unit of an ABAC permission. It tells the platform whether a `subject` is allowed to perform a specific `action` on a specific `object/resource`.

Grants are scoped for fine-grained control by appending a scope after a colon (`:`) in the `action` or `object/resource` name. The default scope (when nothing is specified) is usually `any` for actions and `all` for objects.

**Wenex common action scopes:** `own`, `share`, `group`, `client` (e.g., `create:own`, `update:share`, `restore:group`)

**Wenex object/resource scopes:** collections within services (e.g., `identity:users` — here `users` is the scope).

**Grant Definition Example:**
...
> AI Agent Note: When creating new grants on behalf of a user, always define explicit scopes to follow the principle of least privilege.

User subjects within the Wenex platform are postfixed by the user’s `domain` (a unique FQDN identifier for the token). The domain is taken from `domain/clients.domains[].name` (a nested array field). Each client may support multiple domains, but any single token is associated with exactly one domain at a time.
