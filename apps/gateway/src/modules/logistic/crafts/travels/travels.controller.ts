...
import {
  RoutingDataSerializer,
  RoutingSerializer,
  TravelDataSerializer,
  TravelItemsSerializer,
  TravelSerializer,
} from '@app/common/serializers/logistic';
import { RoutingDto, CreateTravelDto, CreateTravelItemsDto, UpdateTravelDto } from '@app/common/dto/logistic';
...
export class TravelsController extends ControllerClass<Travel, TravelDto> implements IController<Travel, TravelDto> {
  ...
  @SetScope(Scope.ResolveLogisticTravels)
  @ApiResponse({ type: RoutingDataSerializer })
  @SetPolicy(Action.Resolve, Resource.LogisticTravels)
  routing(@Meta() meta: Metadata, @Body() data: RoutingDto): Observable<RoutingDataSerializer> {
    return from(this.provider.travels.routing(data, { meta })).pipe(mapToInstance(RoutingSerializer, 'data'));
  }
